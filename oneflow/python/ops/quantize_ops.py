...
def min_max_observer(
    ...
    r"""Compute the quantization parameters of the input `Blob`.

    First compute the max and min values of input `Blob`:
    ...
    Args:
        ...
        quantize_to_bit (Optional[int], optional): Quantize input to uintX / intX, X can be in range [2, 8]. Defaults to 8. 
        ...
        def QuantizeJob(
            ...
            with flow.scope.placement(device_type, "0:0"):
                scale, zero_point = flow.quantization.MinMaxObserver(
                    input, quantize_to_bit=8, quantize_scheme="symmetric", per_layer_quantize=True
                )
...
def moving_average_min_max_observer(
    ...
    r"""Compute the quantization parameters based on the moving average of the input `Blob`'s min and max values.

    First compute the moving\_max and moving\_min value of input `Blob`:
        ...
        def QuantizeJob(
            ...
            with flow.scope.placement(device_type, "0:0"):
                scale, zero_point = flow.quantization.MovingAverageMinMaxObserver(
                    input, quantize_to_bit=8, quantize_scheme="symmetric", momentum=0.95
                )
            return scale, zero_point
...
def fake_quantization(
    ...
    Returns:
        remote_blob_util.BlobDef: Input `Blob` after quantize and dequantize operations.
        ...
        def QuantizeJob(
            ...
            with flow.scope.placement(device_type, "0:0"):
                ...
                fake_quantize_out = flow.quantization.FakeQuantize(
                    input, scale, zero_point, quantize_to_bit=8, quantize_scheme="symmetric"
                )
